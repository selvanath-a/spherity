...
export class CredentialService {
  ...
  verifyCredential(
    ...
    if (!credential) {
      return Promise.resolve({ valid: false, reason: 'Missing Credential' });
    }
    const { proof, ...rest } = credential;
    if (!proof || !proof.proofValue) {
      return Promise.resolve({ valid: false, reason: 'Missing proof' });
    }
    if (proof.type !== this.EXPECTED_PROOF_TYPE) {
      return Promise.resolve({ valid: false, reason: 'Invalid proof type' });
    }
    if (proof.proofPurpose !== this.EXPECTED_PROOF_PURPOSE) {
      return Promise.resolve({ valid: false, reason: 'Invalid proof purpose' });
    }
    ...
    if (proof.verificationMethod !== expectedVerificationMethod) {
      return Promise.resolve({
        valid: false,
        reason: 'Invalid verification method',
      });
    }
    ...
    if (Number.isNaN(createdTs)) {
      return Promise.resolve({
        valid: false,
        reason: 'Invalid proof created timestamp',
      });
    }
    ...
    if (!isValid) {
      return Promise.resolve({ valid: false, reason: 'Invalid signature' });
    }
